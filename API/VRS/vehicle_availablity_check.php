<?php
    ...
    $vehicle_id = "2";
    $booking_starting_date = "24/05/2023";
    $booking_ending_date = "24/05/2023";
    $ending_date = "";
    $available = false;
    $count = 0;
    ...
    if ($result->num_rows > 0) 
    {
        while($row = $result->fetch_assoc()) 
        {
            ...
            $vehicle_user_id = $row['vehicle_owner_user_id'];

            $result_1 = $connetion->query("SELECT * FROM `renter_{$vehicle_user_id}_current_booking`");
            if ($result_1->num_rows > 0) 
            {
                while($row_1 = $result_1->fetch_assoc()) 
                {

                    $booking_id = $row_1['vehicle_booking_id'];

                    $result_2 = $connetion->query("SELECT * FROM `vehicle_booking` where booking_id = '$booking_id'");
